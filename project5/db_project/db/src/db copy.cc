...
int db_find(int64_t table_id, int64_t key, char* ret_val,uint16_t* val_size, int trx_id){
    ...
    buffer_read_page(table_id,pagenum,&leaf_page);
    ...
    int return_int = trx_acquire(lock,trx_id); // 그 lock을 trx_table의 linked_list에 넣음
    repin(table_id,pagenum);

    //actual operation
    for(int i=0;i<page_header::get_number_of_keys(&leaf_page);i++){///printf("free space : %d\n",leaf_page::get_amount_of_free_space(&key_exist_leaf_page));
        slot_t* ith_slot = leaf_page::get_nth_slot(&leaf_page,i);
        if(ith_slot->get_key() == (pagenum_t)key) {
            ushortint_t size = ith_slot->get_size();
            std::memcpy(val_size,&size,2);
            char* value = leaf_page::get_value(&leaf_page,ith_slot->get_offset(),ith_slot->get_size());
...
//success return 0 else -1
int db_update(int64_t table_id, int64_t key, char* value, uint64_t new_val_size,uint16_t* old_value_size, int trx_id){
    char* original_value = (char*)malloc(sizeof(char)*200);
    if(db_find(table_id,key,original_value,old_value_size,trx_id)==0){ //not found
        free(original_value);
        int i=0;
        ...
        free(ith_slot);
        buffer_write_page(table_id,key_exist_leaf_pagenum,&key_exist_leaf_page);

        //write 해도 되는 이유 -> trx_abort에서 다 되돌릴 예정

        //unpin(table_id,key_exist_leaf_pagenum);
        lock_t* lock = lock_acquire(table_id,key_exist_leaf_pagenum,key,trx_id,Xlock);
        return trx_acquire(lock,trx_id);
    }
...
//printf("Is empty\n");
    ...
    while( check_leaf!= 1){
        ...
        unpin(table_id,key_exist_pagenum);
    }
