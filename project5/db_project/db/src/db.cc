...
int64_t open_table(const char* pathname){
    ...
    table_id = buffer_open_table_file(pathname);
...
int db_insert(int64_t table_id, int64_t key, const char* value, uint16_t val_size){
    ...
    buffer_read_page(table_id,key_exists_leaf_pagenum,&key_exists_leaf_page);
...
int cut(longlong_t table_id,pagenum_t leaf_pagenum){
    ...
    int i,A;
    page_t leaf_page;
    buffer_read_page(table_id,leaf_pagenum,&leaf_page);
...
page_t insert_into_parent(longlong_t table_id,longlong_t leaf_pagenum, pagenum_t new_leaf_pagenum,pagenum_t new_key){
    ...
    buffer_read_page(table_id,leaf_pagenum,&leaf_page);
    buffer_read_page(table_id,new_leaf_pagenum,&new_leaf_page);
    pagenum_t parent_pagenum = page_header::get_parent_page_num(&leaf_page);
    buffer_read_page(table_id,parent_pagenum,&parent_page);
...
page_t insert_into_page_after_splitting(longlong_t table_id,pagenum_t old_pagenum,pagenum_t left_index,pagenum_t new_key,pagenum_t right_pagenum){
    ...
    page_t old_page;
    buffer_read_page(table_id,old_pagenum,&old_page);
    int_4bit A = page_header::get_number_of_keys(&old_page);//key의 개수 /1번째는 1번째 시작 / 248//printf("key_number: %d\n",A);
    //lefmost는 따로 관리
    ...
    //make new page
    page_t new_page;
    pagenum_t new_pagenum = buffer_alloc_page(table_id);
    page_header::set_leaf_check(&new_page,0); //internal
    page_header::set_number_of_keys(&new_page,0);
    buffer_read_page(table_id,new_pagenum,&new_page);
    ...
    for(i=0;i<=page_header::get_number_of_keys(&new_page);i++){
        if(i==0){
            ...
            pagenum_t new_child_pagenum = internal_page::get_leftmost_key(&new_page);
            buffer_read_page(table_id,new_child_pagenum,&new_child_page);
            page_header::set_parent_page_num(&new_child_page,new_pagenum);
            buffer_write_page(table_id,new_child_pagenum,&new_child_page);
            //unpin(table_id,new_child_pagenum);
        }
        else{
            //make new child(leaf or internal)
            page_t new_child_page;
            branching_factor_t* ith_branch_factor = internal_page::get_nth_branching_factor(&new_page,i);
            pagenum_t new_child_pagenum = ith_branch_factor->get_pagenumber();
            buffer_read_page(table_id,new_child_pagenum,&new_child_page);
            page_header::set_parent_page_num(&new_child_page,new_pagenum);
            buffer_write_page(table_id,new_child_pagenum,&new_child_page);
            //unpin(table_id,new_child_pagenum);
...
page_t insert_into_page(longlong_t table_id, pagenum_t parent_pagenum, pagenum_t left_index, pagenum_t new_key,pagenum_t right_pagenum){
    ...
    buffer_read_page(table_id,parent_pagenum,&parent_page);
    buffer_read_page(table_id,right_pagenum,&right_page);//printf("page_header::get_number_of_keys:%d\n",page_header::get_number_of_keys(&parent_page));
    ...
    buffer_write_page(table_id,parent_pagenum,&parent_page);
    //unpin(table_id,parent_pagenum);
...
page_t insert_into_new_root(longlong_t table_id,pagenum_t left_pagenum,pagenum_t key,pagenum_t right_pagenum){
    ...
    buffer_read_page(table_id,left_pagenum,&left_page);
    buffer_read_page(table_id,right_pagenum,&right_page);
    ...
    buffer_write_page(table_id,left_pagenum,&left_page);
    buffer_write_page(table_id,right_pagenum,&right_page);
...
longlong_t get_left_index(longlong_t table_id,pagenum_t parent_pagenum,pagenum_t left_pagenum){//printf("get_left_index\n");
...
page_t insert_into_leaf_after_splitting(longlong_t table_id,pagenum_t leaf_pagenum,longlong_t key, const char* value,ushortint_t val_size){
    ...
    buffer_write_page(table_id,leaf_pagenum,&leaf_page);
    ...
    //unpin(table_id,leaf_pagenum);
    page_t new_leaf;
    pagenum_t new_leaf_pagenum = buffer_alloc_page(table_id);
    buffer_read_page(table_id,new_leaf_pagenum,&new_leaf);
...
page_t insert_into_leaf(longlong_t table_id,pagenum_t leaf_pagenum,longlong_t key, const char* value,ushortint_t val_size){
    ...
    buffer_write_page(table_id,leaf_pagenum,&leaf_page); // 써주는 행위는 각각의 함수가 관리
    //unpin(table_id,leaf_pagenum);
    return leaf_page;
}
...
void start_new_tree_disk(int64_t table_id, int key,const char* value, uint16_t val_size){
    ...
    page_t header_page;//printf("make new tree\n");
    ...
    page_t root_page;
    buffer_read_page(table_id,root_page_num,&root_page);
    //unpin(table_id,root_page_num);
    ...
    ////unpin(table_id,0);
    ////unpin(table_id,root_page_num); -> write //unpin
...
int db_find(int64_t table_id, int64_t key, char* ret_val,uint16_t* val_size, int trx_id){
    ...
    if(pagenum == 0){
        printf("leaf_page does not exist\n");
        return -1;
    }
    /*
    lock_buffer_latch();
    buffer_block* buf = setting_buffer1(table_id,pagenum,&leaf_page);
    pthread_mutex_lock(&buf->page_latch); //
    //pin(table_id,pagenum);
    printf("lock! %d\n",pagenum);
    std::memcpy(&leaf_page, buf->physical_frame,PAGE_SIZE);
    unlock_buffer_latch();*/
    buffer_block* buf = buffer_read_page_trx(table_id,pagenum,&key_exist_leaf_page);
    //buffer_read_page_trx(table_id,pagenum,&leaf_page);

    //int acquire_success = -1;

    //if(lock_exist(table_id,pagenum,key,trx_id) == 0){
    //    acquire_success = 0;
    ...
    //unpin(table_id,pagenum); // release page latch
    lock_t* lock = lock_acquire(table_id,pagenum,key,trx_id,Slock); // lock_table에 lock 넣음 & 그 주소 반환
    int acquire_success = trx_acquire(lock,trx_id); // 그 lock을 trx_table의 linked_list에 넣음
    
    //operation
    if(acquire_success == 0){  
        ...
        buf = buffer_read_page_trx(table_id,pagenum,&key_exist_leaf_page);
        printf("acquire_success : %d",acquire_success);  
        //pin(table_id,pagenum);
        ...
        if(i == page_header::get_number_of_keys(&key_exist_leaf_page)){
            buffer_write_page_trx(buf,table_id,pagenum,&key_exist_leaf_page);
            return -1;
        }
        else{ //find sccess
            slot_t* ith_slot = leaf_page::get_nth_slot(&key_exist_leaf_page,i);
    ...
    return acquire_success;
    /*
    if(db_find(table_id,key,ret_val,val_size) == 0){
        pagenum_t pagenum = find_leaf(table_id,key);
        lock_t* lock = lock_acquire(table_id,pagenum,key,trx_id,Slock); // lock_table에 lock 넣음 & 그 주소 반환
        return trx_acquire(lock,trx_id); // 그 lock을 trx_table의 linked_list에 넣음
    }
    //trx_abort(trx_id);
    return -1; // deadlock detect & abort trx
    */
}
...
int db_update(int64_t table_id, int64_t key, char* value, uint16_t new_val_size,uint16_t* old_value_size, int trx_id){
    printf("db_update!\n");
    //page_t root_page;
    //buffer_block* buffer = buffer_read_page(table_id,0,&root_page);
    ...
    printf("found?\n");
    int i=0;
    //buffer_read_page_trx(table_id,key_exist_leaf_pagenum,&key_exist_leaf_page);
    //lock_buffer_latch();
    /*
    lock_buffer_latch();
    buffer_block* buf = setting_buffer1(table_id,key_exist_leaf_pagenum,&key_exist_leaf_page);
    buf->table_id = table_id;
    buf->pagenum = key_exist_leaf_pagenum;
    pthread_mutex_lock(&buf->page_latch);
    std::memcpy(&key_exist_leaf_page, buf->physical_frame,PAGE_SIZE);
    unpin(table_id,key_exist_leaf_pagenum);*/

    buffer_block* buf = buffer_read_page_trx(table_id,key_exist_leaf_pagenum,&key_exist_leaf_page);
    unpin_buf(buf);
    //unpin(table_id,pagenum); // release page latch
    lock_t* lock = lock_acquire(table_id,key_exist_leaf_pagenum,key,trx_id,Xlock); // lock_table에 lock 넣음 & 그 주소 반환
    int acquire_success = trx_acquire(lock,trx_id); // 그 lock을 trx_table의 linked_list에 넣음
    
    if(acquire_success == 0){
        buf = buffer_read_page_trx(table_id,key_exist_leaf_pagenum,&key_exist_leaf_page);
        //lock_buffer_latch();
        //pin_buf(buf);
        //unlock_buffer_latch();
        ...
        //pthread_mutex_lock(&buf->page_latch);
        //pin(table_id,key_exist_leaf_pagenum);
        ...
        slot_t* ith_slot = leaf_page::get_nth_slot(&key_exist_leaf_page,i);
        *old_value_size = ith_slot->get_size();
        
        ith_slot->set_size(new_val_size);
        leaf_page::set_value(&key_exist_leaf_page,ith_slot->get_offset(),value,ith_slot->get_size()/*new_val_size*/);

        free(ith_slot);

        buffer_write_page_trx(buf,table_id,key_exist_leaf_pagenum,&key_exist_leaf_page);
    }
    else{
        //operation not happen
    }
...
int db_find(int64_t table_id, int64_t key, char* ret_val, uint16_t* val_size){
    ...
    buffer_read_page(table_id,key_exist_leaf_pagenum,&key_exist_leaf_page);
    //unpin(table_id,key_exist_leaf_pagenum);
    for(i=0;i<page_header::get_number_of_keys(&key_exist_leaf_page);i++){///printf("free space : %d\n",leaf_page::get_amount_of_free_space(&key_exist_leaf_page));
        slot_t* ith_slot = leaf_page::get_nth_slot(&key_exist_leaf_page,i);
        if(ith_slot->get_key() == (pagenum_t)key) {
...
pagenum_t find_leaf(int64_t table_id,int64_t key){
    ...
    page_t root_page;
    buffer_read_page(table_id,root_pagenum,&root_page);
...
int db_delete(int64_t table_id, int64_t key){
    ...
    uint16_t value_size;
    int found = db_find(table_id,key,value,&value_size);//printf("%d\n",found);
    pagenum_t found_pagenum = find_leaf(table_id,key);//printf("%d\n",found_pagenum);
...
//printf("root change\n");
        ...
        buffer_read_page(table_id,new_root_pagenum,&new_root_page);
        page_header::set_parent_page_num(&new_root_page,0); 
        header_page::set_root_page_num(&header_page,new_root_pagenum);
        buffer_write_page(table_id,0,&header_page);
        buffer_write_page(table_id,new_root_pagenum,&new_root_page);
    }
    else{ // if leaf page
...
pagenum_t delete_entry(int64_t table_id, pagenum_t key_pagenum, int64_t key){//, char* value){//pagenum_t keys_right_pagenum){
    ...
    buffer_read_page(table_id,key_pagenum,&key_page);
...
//printf("adjust_root_page\n");
    ...
    buffer_read_page(table_id,neighbor_pagenum,&neighbor_page);
...
pagenum_t leaf_merge(int64_t table_id, pagenum_t key_pagenum, pagenum_t neighbor_pagenum, longlong_t neighbor_index, int k_prime){
    ...
    for(i=neighbor_insertion_index,j=0; j < page_header::get_number_of_keys(&key_page);i++,j++){
        ...
        buffer_write_page(table_id,neighbor_pagenum,&neighbor_page);
        free(ith_slot);
    }
    free(ith_offset_slot);

    page_header::set_number_of_keys(&key_page,0);
    buffer_write_page(table_id,key_pagenum,&key_page);
    //이 아래도 공유
...
pagenum_t internal_merge(int64_t table_id, pagenum_t key_pagenum, pagenum_t neighbor_pagenum, longlong_t neighbor_index, int k_prime){
    ...
    for(i=0;i<page_header::get_number_of_keys(&neighbor_page)+1;i++){
        ...
        if(i==0){
            ...
            buffer_read_page(table_id,tmp,&tmp_page);
            page_header::set_parent_page_num(&tmp_page,neighbor_pagenum);
            buffer_write_page(table_id,tmp,&tmp_page);
        }
        else{
...
pagenum_t leaf_redistribute(int64_t table_id, pagenum_t key_pagenum, pagenum_t neighbor_pagenum, longlong_t neighbor_index, longlong_t k_prime_index, int k_prime){
    ...
    buffer_write_page(table_id,key_pagenum,&key_page);
    buffer_write_page(table_id,parent_pagenum,&parent_page); // 사실 안 들어가도 될듯?
...
int need_merge_or_coalesce(int64_t table_id, pagenum_t key_pagenum){
    ...
    buffer_read_page(table_id,key_pagenum,&key_page);
    pagenum_t parent_pagenum = page_header::get_parent_page_num(&key_page);
    buffer_read_page(table_id,parent_pagenum,&parent_page);//printf("parent key 개수 : %d\n",page_header::get_number_of_keys(&parent_page));
    if(page_header::get_leaf_check(&key_page) == 1) { //leaf page이면
...
pagenum_t remove_entry_from_page(int64_t table_id, pagenum_t pagenum, int64_t key){
    int i;
    page_t page; //can be iternal or leaf//printf("remove: %d / from : %d\n",key,pagenum);
...
int db_scan(int64_t table_id, int64_t begin_key,int64_t end_key, std::vector<int64_t>* keys,std::vector<char*>* values,std::vector<uint16_t>* val_sizes){
    ...
    while(leaf_page::get_right_sibling_page_number(&key_exist_leaf_page) != 0){
        ...
        buffer_read_page(table_id,leaf_page::get_right_sibling_page_number(&key_exist_leaf_page),&key_exist_leaf_page);
        i = 0;
    }
    free(ith_slot);
    return 0;
...
int init_db(int num_buf){
    //return 0;
    int i = buffer_init_db(num_buf);//printf("buffer_init_db: %d\n",i);
    init_trx_table();
    init_lock_table();
    return i;
    //destroy tree
