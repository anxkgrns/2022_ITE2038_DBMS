...
//test0
TEST(db_insert_Check, insert_test) {
    ...
    int key_num = 100000;
    //uint16_t value_size = 100; // value의 길이
...
TEST(db_insert_Check, RandomValueAndRandomKey) {
    ...
    //Do insertion key_num times
    for(int i=0;i<key_num;i++){
        values[i] = get_random_value(value_size,key[i]);
        int check_inserted = db_insert(table_id,key[i],values[i],value_size);
        EXPECT_EQ(check_inserted,0)
            << "failed inserting " << key[i] << " in " << i+1 << "th insertion \n";
    }
    ...
    for(int i=0;i<key_num;i++){
        ...
        EXPECT_EQ(db_find(table_id,key[i],find_value,&find_value_size),0)
            << i+1 << "th find failed\n";
        //check_size
        EXPECT_EQ(value_size,find_value_size);
        std::string str_value(values[i],value_size);
        std::string found_value(find_value,find_value_size);
        //check string
        EXPECT_EQ(str_value,found_value);
        free(find_value);
